...
export default function Popup({
  ...
  return (
    <Dialog open={isShow} onOpenChange={handleToggle}>
      <DialogContent
        className={twMerge(
          'flex flex-col overflow-y-hidden w-[90%] max-h-[90vh] max-w-md sm:max-w-lg  bg-bolt-elements-background-depth-3 text-center',
          className,
        )}
      ...
      >
        ...
        >
          ...
          <div className={twMerge('flex-1 min-h-0 overflow-y-auto', childrenClasses)}>{children}</div>
        </motion.div>
      </DialogContent>
    </Dialog>
  );
