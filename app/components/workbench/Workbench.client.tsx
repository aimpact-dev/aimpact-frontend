...
export const Workbench = memo(
  ({ chatStarted, isStreaming, actionRunner, metadata, updateChatMestaData, postMessage }: WorkspaceProps) => {
    ...
    useEffect(() => {
      ...
      const removeSubscribe = getWorkbenchStore().files.listen((files) => {
    ...
    function getFirstActionRunner(): ActionRunner | null {
      const artifacts = Object.values(getWorkbenchStore().artifacts.get());
    ...
    function getPackageJson(): PackageJson | null {
      try {
        return getWorkbenchStore().getPackageJson();
      } catch (e) {
    ...
    useEffect(() => {
      ...
      const processPreviewStart = async (): Promise<void> => {
        ...
        const shouldContinue = () => {
          const previewAvailable = getWorkbenchStore().previews.get().length > 0;
          const previewTabOpen = getWorkbenchStore().currentView.get() === 'preview';
        ...
        while (shouldContinue()) {
          ...
          if (installationRunningOrPending(packageJson, getWorkbenchStore().getMainShell)) {
          ...
          if (!previewCommandIsRunningOrPending(packageJson, getWorkbenchStore().getMainShell)) {
            getWorkbenchStore().getMainShell.executeCommand(getPreviewStartCommand(packageJson)).catch((err) => {
              console.error(err);
