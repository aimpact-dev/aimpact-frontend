...
import {
  useGetLeaderboardTopQuery,
  useGetLeaderboardPositionQuery,
} from "query/use-leaderboard-query"
...
export default function Leaderboard() {
  ...
  } = useGetLeaderboardTopQuery(jwtToken)
  const {
    data: userPositionData,
    error: userPositionError,
  } = useGetLeaderboardPositionQuery(jwtToken)
      ...
      <div className="container mx-auto px-6 py-12">
        ...
        <Card className="bg-gray-900/50 border-gray-700 backdrop-blur-sm">
          ...
          <CardContent>
            ...
            {/* show outside top */}
            {userPositionData && (
              <div className="grid grid-cols-3 gap-4 p-4 px-6 mt-6 rounded-lg
                bg-[#9987ee1a] border border-[#9987ee]">
                <div className="flex items-center">
                  <Badge
                    variant={getRatingBadgeVariant(
                      userPositionData.points
                    )}
                    size="lg"
                    className="w-8 h-8 flex items-center justify-center font-bold"
                  >
                ...
                <div className="flex items-center justify-end">
                  <span className="text-white font-semibold text-lg">
                    {userPositionData.points.toLocaleString()}
                  </span>
                </div>
              </div>
            )}
